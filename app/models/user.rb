  ...
  def self.new_with_session(params, session)
    super.tap do |user|
      if session["devise.facebook_data"]
        ...
        user.avatar = open(session["devise.facebook_data"]["info"]["image"]) if user.avatar.blank?
        #user.image=URI.parse session["devise.facebook_data"]["info"]["image"] if user.image.blank?
      end
    end
  end
